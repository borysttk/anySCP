//! Credential vault — secure storage for host passwords and key passphrases.
//!
//! The concrete store is chosen at compile time:
//!
//! | Target  | Backend                                                      |
//! |---------|--------------------------------------------------------------|
//! | macOS   | Keychain (`keyring`, apple-native)                           |
//! | Windows | Credential Manager (`keyring`, windows-native)               |
//! | Linux   | kernel keyutils (`keyring`, linux-native)                    |
//! | Android | `EncryptedSharedPreferences` + Keystore, via JNI             |
//!
//! `keyring` 3.x has no Android backend, so Android routes through
//! [`android_store`] instead. Both backends expose the same four functions and
//! the same [`VaultError`] variants, so every caller — and the three Tauri
//! commands below — is platform-agnostic.
...
mod android_store;
#[cfg(target_os = "android")]
use android_store as backend;

// Re-exported so existing call sites (`vault::save_credential`, …) keep working
// unchanged across all six modules that use them.
pub use backend::{delete_credential, get_credential, has_credential, save_credential};
...
pub(crate) const SERVICE_NAME: &str = "com.anyscp.credentials";
...
// ---------------------------------------------------------------------------
//
// All three commands delegate to `spawn_blocking` because both backends
// perform synchronous work: the desktop keychain does IPC to the OS credential
// store, and the Android backend makes blocking JNI calls. Blocking the tokio
// executor thread even briefly would degrade all concurrent tasks.

/// Save or replace the credential for a host in the platform secure store.
#[tauri::command]
...
/// Delete the credential for a host from the platform secure store.
#[tauri::command]
...
// These exercise the `keyring` backend through an in-memory credential
// builder, so they only apply to the desktop build. The Android backend
// needs a live JVM and is covered by instrumented tests on device.
